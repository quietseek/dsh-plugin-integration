...
/**
 * Quote one YAML scalar. Plain-safe text stays bare, but anything a YAML parser
 * would read back as a number, boolean or null gets quoted — the mcp-client
 * Config validates `args`, `env` and `headers` values as strings, so an
 * unquoted `9000` would arrive as a number and fail the row at activation.
 */
function yamlScalar(value) {
  const text = String(value === null || value === undefined ? '' : value)
  if (text.length === 0) return "''"
  const plainSafe = /^[A-Za-z0-9_.\/@:+-]+$/.test(text)
  const reserved = /^(?:true|false|yes|no|on|off|null|none|nan|inf)$/i.test(text)
  const numeric = /^[+-]?(?:\d|\.\d)/.test(text)
  if (plainSafe && !reserved && !numeric) return text
...
function mcpSnippet(input) {
  ...
  const lines = []
  lines.push('- insert:')
  lines.push('    - id: ' + (serverName.length > 0 ? serverName : 'server'))
  lines.push("      name: '@deepseek-ai/dsh-mcp-client'")
  lines.push('      config:')
  lines.push('        serverName: ' + (serverName.length > 0 ? serverName : 'server'))
  ...
  if (transport === 'stdio') {
    ...
    if (envList.length > 0) {
      lines.push('        env:')
      for (const item of envList) lines.push('          ' + item.name + ': ' + yamlScalar(item.value))
    }
  } else {
    ...
    if (headerList.length > 0) {
      lines.push('        headers:')
      for (const item of headerList) lines.push('          ' + item.name + ': ' + yamlScalar(item.value))
    }
  }
...
async function applyMcpEntry(input, runtime) {
  ...
  try {
    ...
    if (current.text.indexOf(markerFor(serverName)) !== -1) {
...
async function replaceMcpEntry(input, runtime) {
  ...
  const rendered = mcpSnippet(source)
  if (!rendered.ok) return { ok: false, errors: rendered.errors, snippet: rendered.snippet }
  const denied = profileWriteError(runtime)
  if (denied !== null) return { ok: false, errors: [denied], snippet: rendered.snippet }
  const patchPath = runtime.profile.patchPath
...
async function handleRequest(runtime, req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (action === 'mcp-snippet') return sendJson(res, 200, mcpSnippet(snippetInputFrom(url.searchParams)))
